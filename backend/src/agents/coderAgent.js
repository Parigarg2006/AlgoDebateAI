...
6. Do NOT make flawed mathematical closed-form assumptions (like assuming a simple formula exists) or use guessed closed-form formulas (e.g., 'n % 2 != 0' or 'n/2 * m'). Always write complete, verified dynamic evaluation (DP), memoized state machine transitions, or simulation-based logic that covers ALL alternating state transitions and constraints under all input variations. For alternating sequence logic, explicitly account for the strict alternating constraint where an UP step can add at most +M, but the mandatory DOWN step must subtract at least 1 (or follow full DP bounds) before the next peak (e.g., for N=4, S=3, M=5, the optimal alternating sequence is [3, 8, 7, 12] yielding the maximum value 12). Do not use guessed closed-form shortcuts.
  `.trim();
