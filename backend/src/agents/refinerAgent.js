...
6. FULL IMPLEMENTATION MANDATE: Ensure that 'finalCode' contains the COMPLETE, FULLY IMPLEMENTED ALGORITHMIC SOLUTION for ${langUpper}. Under NO circumstances return an empty stub, 'pass', 'return null;', or boilerplate shell.
  ...
  try {
    ...
    const dynamicExplanation = (debateHistory.length > 0 && debateHistory[debateHistory.length - 1].criticism) 
      ? debateHistory[debateHistory.length - 1].criticism.replace(/\[SANDBOX FAILURE\].*/s, '').trim()
      : 'Optimal algorithm implementation verified by multi-agent consensus.';

    return {
      finalCode: cleanCodeString(finalDraftCode),
      explanation: dynamicExplanation || 'Optimal solution code generated by the agent debate loop.',
      timeComplexity: 'O(N)',
      spaceComplexity: 'O(1)'
    };
  }
